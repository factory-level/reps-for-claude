...
import { db } from './db';
import { activity, subscriber, syncBody, query } from './validation';
...
export async function handle(req:Request):Promise<Response>{
 ...
 if(path==='activity' && req.method==='GET'){
  ...
  const [rows,featured]=await Promise.all([
   sql`select a.id::text,a.nickname,a.exercise,a.quantity::float8,a.unit,a.note,a.created_at,a.featured,(select count(*)::int from reps.cheers c where c.activity_id=a.id) cheers from reps.activities a where (${q.cursor}=0 or a.id<${q.cursor}) order by a.id desc limit ${q.limit+1}`,
   sql`select a.id::text,a.nickname,a.exercise,a.quantity::float8,a.unit,a.note,a.created_at,a.featured,(select count(*)::int from reps.cheers c where c.activity_id=a.id) cheers from reps.activities a where a.featured order by a.id desc limit 3`
  ]);
  return Response.json({items:rows.slice(0,q.limit),featured,nextCursor:rows.length>q.limit?rows[q.limit-1].id:null,configured:true},{headers:{'Cache-Control':'public, max-age=0, s-maxage=60, stale-while-revalidate=300'}});
 }
 if(path==='activity' && req.method==='POST'){
  await visitor(req,'post',10); const v=activity.parse(await body(req)),token=randomBytes(32).toString('base64url');
  const rows=await db()`insert into reps.activities(nickname,exercise,quantity,unit,note,deletion_hash) values(${v.nickname},${v.exercise},${v.quantity},${v.unit},${v.note},${hash(token)}) returning id::text`;
  return json({id:rows[0].id,deletionToken:token},201);
 }
  ...
  await db().begin(async sql=>{
   ...
   if(owner.autopost && owner.public_after){
    ...
    for(const row of pending){
     const r=row.record;
     await sql`insert into reps.activities(nickname,exercise,quantity,unit,note,deletion_hash,source_key)
      values(${owner.nickname},${r.exercise},${r.kind==='REP'?r.reps:r.seconds},${r.kind==='REP'?'reps':'seconds'},${'A little movement between prompts.'},${hash(randomBytes(32).toString('hex'))},${dataset+':'+row.id}) on conflict(source_key) do nothing`;
